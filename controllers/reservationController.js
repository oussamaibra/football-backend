...
exports.createReservation = async (req, res) => {
  ...
  const endTime = new Date(startTime.getTime() + durationMinutes * 60000 + 10 * 60000); // inclure 10 minutes de pause
  ...
  try {
    ...
    if (rows.length > 0) {
      return res.status(400).json({ error: 'Terrain déjà réservé pour ce créneau.' });
    }
    ...
    const insertQuery = `
      ...
      VALUES (?, ?, ?, ?, ?)
    `;
    await db.execute(insertQuery, [userId, court_id, reservation_date, start_time, duration_hours]);
